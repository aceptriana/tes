...
// Fetch saldo awal from rekening_bank
$saldo_awal_sql = "SELECT saldo FROM rekening_bank LIMIT 1";
$saldo_awal_result = $conn->query($saldo_awal_sql);
if ($saldo_awal_result->num_rows > 0) {
    $saldo_awal_row = $saldo_awal_result->fetch_assoc();
    $saldo_awal = $saldo_awal_row['saldo'];
}
        ...
        <div class="card mt-4">
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-md-3">
                        <h6>Saldo Awal:</h6>
                        <p>Rp. <?php echo number_format($saldo_awal, 2, ',', '.'); ?>,-</p>
                    </div>
                </div>
        ...
        <div class="card mt-4">
            <div class="card-body">
                <div class="table-responsive"> <!-- Add this for responsive table -->
                    <table class="table table-striped table-bordered table-hover"> <!-- Add striped and bordered classes -->
                        <thead class="thead-light"> <!-- Add a light background for header -->
                            <tr>
                                <th>No</th>
                                <th>Tanggal</th>
                                <th>Kategori</th>
...
if (!empty($transactions)) {
    ...
    foreach ($transactions as $row) {
        echo "<tr>
                <td>{$no}</td>
                <td>{$row['tanggal']}</td>
                <td>{$row['nama_akun']}</td> <!-- Mengganti kode_akun dengan nama_akun -->
    ...
    $saldo_akhir = $saldo_awal + $total_pemasukan - $total_pengeluaran; // Hitung saldo akhir
    echo "<tr>
