...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $kode_preorder_supplier = htmlspecialchars(trim($_POST['kode_preorder_supplier']));
    ...
    // Validation
    if (empty($kode_preorder_supplier) || empty($kode_penyimpanan) || 
        empty($tanggal_pesan) || empty($nama) || 
        empty($jenis) || empty($warna) || $jumlah === null || $panjang === null || $roll === null) {
        header("Location: tambah-po-supplier.php?status=error&message=Semua field wajib diisi dengan benar");
        exit;
    }

    // Prepared statement to prevent SQL Injection
    $sql = "INSERT INTO preorder_supplier (kode_preorder_supplier, kode_penyimpanan, tanggal_pesan, tanggal_dikirim, nama, jenis, warna, jumlah, panjang, roll, deskripsi_barang, gambar_barang) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    if ($stmt = $conn->prepare($sql)) {
        ...
        if ($stmt->execute()) {
            header("Location: data-po-supplier.php?status=success&message=Data pre-order supplier berhasil ditambahkan");
            exit;
        } else {
