...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $kode_prapesan = htmlspecialchars(trim($_POST['kode_prapesan']));
    $kode_pemasok = htmlspecialchars(trim($_POST['kode_pemasok']));
    ...
    if (isset($_FILES['gambar_barang']) && $_FILES['gambar_barang']['error'] === UPLOAD_ERR_OK) {
        ...
        $maxsize = 5 * 1024 * 1024;
        if ($filesize > $maxsize) {
            die("ErroFFr: Ukuran file lebih besar dari batas yang diizinkan (5MB).");
        }
  ...
  // Validation
  if (empty($kode_prapesan) || empty($kode_pemasok) || empty($kode_penyimpanan) || 
  empty($tanggal_pesan) || empty($nama) || empty($jenis) || 
  empty($warna) || $jumlah === null || $panjang === null || $roll === null) {
  header("Location: tambah-po.php?status=error&message=Semua field wajib diisi dengan benar");
  exit;
}

// Prepared statement to prevent SQL Injection
$sql = "INSERT INTO prapesan (kode_prapesan, kode_pemasok, kode_penyimpanan, tanggal_pesan, 
      tanggal_dikirim, nama, jenis, warna, jumlah, panjang, roll, deskripsi_barang, gambar_barang) 
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

if ($stmt = $conn->prepare($sql)) {
  $stmt->bind_param('ssssssssiidss',  
      $kode_prapesan,
      $kode_pemasok,
      $kode_penyimpanan,
      $tanggal_pesan,
  ...
  if ($stmt->execute()) {
      header("Location: data-po.php?status=success&message=Data pre-order berhasil ditambahkan");
      exit;
  } else {
